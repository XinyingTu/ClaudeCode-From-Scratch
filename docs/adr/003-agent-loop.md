...
Perform all reasoning in a single request. Rejected because the model cannot reason over information that has not yet been observed.

### Option 2 — Iterative Agent Loop

Alternate between reasoning, acting, and observing until the task is complete. Accepted because every observation expands the agent's understanding of the environment, enabling more informed decisions in subsequent iterations.

## First Principles

**Single Responsibility** (Separation of Concerns, SRP)

The Agent Loop coordinates the reasoning cycle but does not reason itself. All reasoning is delegated to the LLM. Tool execution is delegated to the Tool Registry.

**Depend on Stable Abstractions** (Dependency Injection, decoupling)

The Agent Loop depends on the Tool Registry and LLM interfaces, not on concrete implementations. Dependencies are injected at construction time.

## Next Challenge

The Agent Loop accumulates observations across iterations. The next challenge is representing and managing the working state of a task without exposing message-storage details to the orchestration layer.
